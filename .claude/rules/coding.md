...
- `UrlFetchApp.fetch` を直接呼ばず、用途に応じたラッパーを使っているか？
  - Gemini API・LINE送信 → `fetchWithRetry()`（最大3回リトライ）
  - RSSフィード取得 → `UrlFetchApp.fetchAll()`（並列フェッチ＋失敗分1回リトライ）
  - リダイレクト解決 → `resolveRedirectUrl()` 内で直接呼び出し（リトライなし・フォールバックあり）
- `muteHttpExceptions: true` を設定し、HTTPエラーをコードで処理しているか？
- `SpreadsheetApp.getActiveSpreadsheet()` は関数の先頭で1回だけ呼んでいるか？（ループ内で呼ばない）

## 外部通信の実装チェックリスト

新たに `UrlFetchApp` による外部通信を追加・変更するとき：

- リトライの要否を検討し、実装するかどうか判断したか？（判断基準: 冪等でない通信はリトライの副作用を確認する）
- `architecture.md` の「外部通信のリトライ」表を更新したか？
