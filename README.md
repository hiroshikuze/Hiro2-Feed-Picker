...
* GEMINI API KEY
* LINE Messagingに関する、ACCESS TOKEN（チャネルアクセストークン）・OWNER USER ID（あなたのユーザーID）
...
先頭に `-`（半角ハイフン）を付けたキーワードは**マイナスキーワード**として扱われます。

正キーワードに一致しても、マイナスキーワードも同時に含まれる記事は除外されます。
...
上記の例では「ケーキ」「フルーツ」「ベーグル」のいずれかを含む記事が抽出対象になりますが、「バナナ」を含む記事は除外されます。
...
1. **スクリプトプロパティの設定**:
   ...
   * `スクリプト プロパティ` セクションで、以下の4つのプロパティを追加・設定します。
   ...
   初回実行後、以下のプロパティがスクリプトによって自動的に追加されます。手動での設定は不要です。

     * `SENT_ARTICLE_URLS`: 重複送信を防ぐために直近100件の送信済み記事URLをJSON配列で保存します。削除するとその後初回に送信済みの記事が再送される可能性があります。
...
1. **（任意）Google News URLの動作確認**:

   Google NewsのRSSを購読している場合、URLが自動で実際の記事URLに変換されます（`news.google.com/rss/articles/...` → 元記事のURL）。この変換はGoogle内部APIを利用しているため、仕様変更により動作しなくなる可能性があります。変換が機能しているか確認したい場合は以下の手順で行ってください。

   * `<> エディタ` > `main.js`を開きます。
   * 関数の選択欄（デフォルト `main` と表示されている箇所）で `testGoogleNewsUrlResolution` を選択します。
   * `▷ 実行` をクリックし、管理者LINEに「✅ Google News URL解決成功」が届けば正常です。
   * 「⚠️ 未解決（フォールバック）」と届いた場合、変換できなかった記事はGoogle News URLのままLINEに送られます（リンク切れになる可能性があります）。
...
[![GitHub Stars](https://img.shields.io/github/stars/hiroshikuze/hiro2-feed-picker?style=for-the-badge&logo=github&color=gold&label=Stars)](https://github.com/hiroshikuze/hiro2-feed-picker/stargazers)
